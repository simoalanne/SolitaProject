...
type FundingData = Record<string, number>;

const json: FundingData = csvContent
  .split("\n")
  .slice(1)
  .reduce((acc: FundingData, csvRow: string) => {
    // regex is equivalent to splitting by comma but respecting quoted commas
    const allFields = csvRow.match(/(".*?"|[^",]+)(?=\s*,|\s*$)/g);
    if (!allFields) return acc;

    const businessId = allFields[1];
    const totalFunding = Number(
      allFields[allFields.length - 1].replace(/\s/g, "")
    );
    if (!totalFunding) return acc;

    const existingFunding = acc[businessId] || 0;
    acc[businessId] = existingFunding + totalFunding;
    return acc;
  }, {} as FundingData);
